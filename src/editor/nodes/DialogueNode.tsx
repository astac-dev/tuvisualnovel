...
export const DialogueNode: React.FC<NodeProps> = ({ id, data, selected }) => {
    ...
    <div className={`w-48 bg-slate-800 border-2 rounded-xl shadow-lg transition-colors overflow-hidden ${borderClass}`}>
      ...
      <div className="bg-blue-900/60 px-3 py-2 flex items-center justify-between border-b border-blue-500/30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-blue-100 tracking-wide uppercase">Dialogue</span>
        </div>
      ...
      <div className="p-3 flex flex-col gap-2">
        <div className="text-xs font-bold text-blue-200 truncate">{String(data.speaker || 'Narrador')}</div>
        <div className="text-xs text-slate-300 line-clamp-3 bg-slate-900/50 p-2 rounded border border-slate-700/50">
          {String(data.text || '...')}
        </div>
        
        <div className="text-xs text-slate-400 font-medium mt-1">Preview</div>
        <div className="text-xs text-slate-300 italic truncate opacity-80">
          "{String(data.text || '...')}"
        </div>
      </div>
